...
public sealed record ProductDetailsQuery(
    int UserId,
    int ProductId,
    int TableId) : IPrimitiveResultQuery<ProductDetailsDbQueryResponse>;
...
sealed class ProductDetailsQueryHandler : IPrimitiveResultQueryHandler<ProductDetailsQuery, ProductDetailsDbQueryResponse>
{
    ...
    {
        var pollyContext = new Context($"{this.GetType().FullName}|{request.ProductId}");
        return await this._asyncRequestCollapserPolicy.ExecuteAsync(_ =>
            this.HandleCore(request, cancellationToken), pollyContext)
           .ConfigureAwait(false);
    }

    public async Task<PrimitiveResult<ProductDetailsDbQueryResponse>> HandleCore(
